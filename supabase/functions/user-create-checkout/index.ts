...
export default Deno.serve(async (req: Request) => {
  ...
  const client = createClient(url, key, { global: { headers: { Authorization: req.headers.get("Authorization") || "" } } });
  const { plan_id } = await req.json();
  const { data: user } = await client.auth.getUser();
  const uid = user?.user?.id;
  if (!uid || !plan_id) return new Response(JSON.stringify({ error: "missing_params" }), { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } });
  ...
  if (planErr || !plan) return new Response(JSON.stringify({ error: planErr?.message || "plan_not_found" }), { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } });
  ...
  if (subErr || !sub) return new Response(JSON.stringify({ error: subErr?.message || "sub_create_failed" }), { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } });
  ...
  return new Response(JSON.stringify({ subscription_id: sub.id, preference_id: pref?.id, init_point: pref?.init_point }), { headers: { "Content-Type": "application/json", ...corsHeaders } });
});
